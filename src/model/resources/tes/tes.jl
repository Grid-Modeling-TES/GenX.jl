...
If there is more than one representative period and ```LDS``` has been enabled for resources in ```Generators.csv```, this function calls ```long_duration_storage_tes()``` in ```long_duration_storage_tes.jl``` to enable this feature.
...
\end{aligned}
```
The above constraints are established in ```storage_all_tes!()``` in ```storage_all_tes.jl```.
