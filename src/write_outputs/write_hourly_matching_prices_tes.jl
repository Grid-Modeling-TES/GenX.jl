function write_hourly_matching_prices_tes(path::AbstractString,
        inputs::Dict,
        setup::Dict,
        EP::Model)
    ...
    dfHourlyMatchPrices = hcat(dfHourlyMatchPrices,
        DataFrame(
            dual.(EP[:cHourlyMatching_TES]).data ./ transpose(inputs["omega"]) *
            scale_factor,
            :auto))
    ...
    CSV.write(joinpath(path, "hourly_matching_prices_tes.csv"),
